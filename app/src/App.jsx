...
const PASOS = ["Datos Básicos", "Programa y Mapa", "Secuencias", "Instrumentos", "Coherencia y Exportar"];
const TIPS = [
...
export default function App() {
  ...
  const [tema, setTema] = useState(() => localStorage.getItem(LS_TEMA) || "claro");
  ...
  return (
    <div className="app" data-tema={tema}>
      <header className="topbar">
        ...
        <div className="meta">
          <span className="pill ok"><i />Guardado local</span>
          <span className="pill">Llamadas hoy: <b>{llamadas}</b></span>
          <button
            type="button"
            className="tema"
      ...
      <nav className="stepper">
        {PASOS.map((p, i) => (
          <button
            key={p}
            type="button"
            className={i === paso ? "activo" : i < paso ? "listo" : ""}
            onClick={() => ir(i)}
          >
            <span className="num">{i < paso ? "✓" : i + 1}</span>
      ...
      <div className="layout">
        <section className="card">
          {paso === 0 && (
            <>
              <h2>Paso 1: Ficha Técnica de la Unidad</h2>
              <p className="desc">Datos institucionales para contextualizar las propuestas de la IA.</p>
              <label>Nombre de la Unidad de Competencia (Asignatura) *
              ...
              <div className="fila">
                ...
                <label>Horas con Docente (HPC)
                  <input type="number" min="0" value={datos.hpc} onChange={set("hpc")} placeholder="48" />
                </label>
                <label>Horas Independientes (HPS)
                  <input type="number" min="0" value={datos.hps} onChange={set("hps")} placeholder="32" />
                </label>
              </div>
              <span className="etiqueta">Modalidad de Creación Curricular</span>
              <div className="fila">
                <label className={`opcion ${datos.modo === "desde-cero" ? "sel" : ""}`}>
                  <input type="radio" name="modo" checked={datos.modo === "desde-cero"} onChange={() => setDatos((d) => ({ ...d, modo: "desde-cero" }))} />
                  <div><b>Desde Cero con IA</b><small>Generación completa. El resultado sale como borrador.</small></div>
                </label>
                <label className={`opcion ${datos.modo === "partir-de" ? "sel" : ""}`}>
                  <input type="radio" name="modo" checked={datos.modo === "partir-de"} onChange={() => setDatos((d) => ({ ...d, modo: "partir-de" }))} />
                  <div><b>Partir de Programa Oficial</b><small>Respeta tu documento y completa lo faltante.</small></div>
                </label>
              </div>
              ...
              {anexos.length > 0 && <ul className="files">{anexos.map((a) => <li key={a.nombre}>{a.nombre}</li>)}</ul>}
            </>
          )}

          {paso === 1 && (
            <>
              <h2>Paso 2: Programa y Mapa</h2>
              <p className="desc">Genera el programa descriptivo y edítalo: lo validado alimenta los siguientes pasos.</p>
              <label>Pedido al planeador (opcional si anexaste programa)
                <textarea value={pedido} onChange={(e) => setPedido(e.target.value)} placeholder="Genera el programa descriptivo y el mapa de la unidad…" />
              </label>
              <div className="acciones">
                <button type="button" className="primario" disabled={cargando} onClick={() => generar("planeador-unidad", contextoBase, "programa")}>
          ...
          {paso === 2 && (
            <>
              <h2>Paso 3: Secuencias</h2>
              <p className="desc">Genera una subcompetencia por vez; puedes acumular varias sin perder lo anterior.</p>
              <label>Subcompetencia a planear (número, propósito, resultados y horas)
                <textarea value={pedido} onChange={(e) => setPedido(e.target.value)} placeholder="Subcompetencia 1… Resultados 1.1 (5 h)…" />
              </label>
              <label className="drop">
              ...
              <label>Secuencias acumuladas (edítalas)
          ...
          {paso === 3 && (
            <>
              <h2>Paso 4: Instrumentos de Evaluación</h2>
              <p className="desc">Rúbricas, listas de cotejo o pruebas alineadas a tus evidencias y porcentajes.</p>
              <label>Resultado(s) a evaluar (actividades, evidencias y % del cuadro)
                <textarea value={pedido} onChange={(e) => setPedido(e.target.value)} placeholder="Resultado 1.3… Evidencias… %: 5…" />
              </label>
              <div className="acciones">
                <button
                  type="button"
          ...
          {paso === 4 && (
            <>
              <h2>Paso 5: Coherencia y Exportar</h2>
              <p className="desc">Verifica antes de entregar: ¿las horas suman? ¿los % cuadran? ¿toda evidencia tiene instrumento?</p>
              <div className="acciones">
                <button type="button" className="primario" onClick={() => window.print()}>Imprimir / PDF</button>
                <button
                  type="button"
                  onClick={() => navigator.clipboard.writeText(`# ${datos.unidad}\n\n${datos.programa}\n\n${datos.secuencias}\n\n${datos.instrumentos}`)}
                >
                  Copiar todo
                </button>
                <button
                  type="button"
                  onClick={() => descargar("paquete-curricular.md", `# ${datos.unidad}\n\n${datos.programa}\n\n${datos.secuencias}\n\n${datos.instrumentos}`)}
                >
                  Descargar .md
                </button>
              </div>
          ...
          <footer className="navpaso">
            <button type="button" disabled={paso === 0} onClick={() => ir(paso - 1)}>← Atrás</button>
            <span>Paso {paso + 1} de 5</span>
            {paso < 4 && <button type="button" className="primario" onClick={() => ir(paso + 1)}>Continuar →</button>}
          </footer>
        </section>
        ...
        <aside className="lateral">
          <div className="panel resumen">
            ...
            <dl>
              <div><dt>Horas con Docente (HPC)</dt><dd>{datos.hpc || "—"}</dd></div>
              <div><dt>Horas Independientes (HPS)</dt><dd>{datos.hps || "—"}</dd></div>
              <div className="total"><dt>Total semestre</dt><dd>{totalHoras ? `${totalHoras} hrs` : "—"}</dd></div>
            </dl>
          </div>
          <div className="panel ayuda">
            <h3>En este paso</h3>
            <p>{TIPS[paso]}</p>
            {datos.unidad && <p className="ctx"><b>Unidad:</b> {datos.unidad}</p>}
          </div>
        </aside>
      </div>
    </div>
